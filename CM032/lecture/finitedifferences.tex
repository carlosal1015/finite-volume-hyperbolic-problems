...
\begin{frame}
	...
	Further, approximate the second spatial derivative by the second
	...
	\end{equation*}
	etc.
	As a result we arrive at the Crank-Nicolson scheme:
	\begin{equation}\label{eq:heatPDECrankNicolson}
		...
		\end{cases}
	\end{equation}
	Here $f^{m-\frac{1}{2}}_{j}=f\left(x_{j},t_{m-\frac{1}{2}}\right)$,
	which can be replaced by $\left(f^{m}_{j}+f^{m-1}_{j}\right)/2$.
	The difference equation (6.1.16) can be rewritten as
	\begin{equation*}
