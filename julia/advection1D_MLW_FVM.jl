using OffsetArrays
"""
Ecuación de transporte. MVF. Método Lax-Wendroff.
...
"""
function advection1D_MLW_FVM(xi, dt, nt, c, u0)
    nx = length(u0)
    dx = xi[2] - xi[1]
    ...
    for n = 0:nt-1
        ...
        u[nx+1, n] = u[1, n]
        for i = 1:nx
            u[i, n+1] = u[i, n] - (r / 2) * (u[i+1, n] - u[i-1, n]) + ((r^2) / 2) * (u[i+1, n] - 2 * u[i, n] + u[i-1, n])
        end
    end
    return u[1:nx, :]
end