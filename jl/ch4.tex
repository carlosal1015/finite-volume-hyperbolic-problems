...
Sea $\left(\Omega,\sigma,\mathbb{P}\right)$ un espacio probabilístico y $X_1, \dots, X_d : \Omega \to \mathbb{R}$ variables aleatorias independientes dos a dos con $E(X_l) = 0$ y momentos de orden $k$ acotados por $|E(X_l^k)| \leq \frac{k!}{2}$ para todo $l = 1, \dots, d$ y $k \geq 2$. En estas condiciones, para todo $\epsilon > 0$, se tiene que:
\[ P [ |X_1 + \dots + X_d| \geq \epsilon ] \leq 2e^{-\frac{1}{4} \min(\frac{\epsilon^2}{d}, \epsilon)}. \]
\end{lemma}

\begin{theorem}[Teorema del Anillo Gaussiano]
Sean $\left(\Omega,\sigma,\mathbb{P}\right)$ un espacio probabilístico, $X : \Omega \to \mathbb{R}^d$ una variable aleatoria con distribución $\mathcal{N}(0_d, I_d)$ y $0 \leq \epsilon \leq \sqrt{d}$. Entonces se cumple que
