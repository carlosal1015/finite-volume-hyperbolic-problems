...
def CL():
    ...
    # fracdata = [
    #            [0.5,0.005, 0.5,        0.5, 0.3],
    #            [0.5,0.005, math.pi/2,  0.5, 0.5],
    #            [0.3,0.005, 0.5,        0.2, 0.8]
    #           ]
    fracdata = [
        [0.2, FracWidth, 0.3, 0.2, 0.8],
        [0.2, FracWidth, -0.75, 0.8, 0.3],
        [0.2, FracWidth, 1.3, 0.4, 0.7],
        [0.1, FracWidth, -0.5, 0.2, 0.5],
        [0.2, FracWidth, 0.25, 0.7, 0.6],
        [0.2, FracWidth, -2.1, 0.2, 0.2],
        [0.1, FracWidth, 1.0, 0.8, 0.9],
        [0.2, FracWidth, 0, 0.6, 0.1],
    ]
    ...
    for cell in cells(mesh):
        ...
        perm[cell] = perm_ref * pow(
            pow(poro[cell] / poro_ref, 3) * pow((1 - poro_ref) / (1 - poro[cell]), 2), 6
        )
    ...
    c = CompiledExpression(
        compile_cpp_code(property_code).Property(), c0=poro, c1=perm, degree=0
    )
...
def SB():
    ...
    c = CompiledExpression(
        compile_cpp_code(property_code).Property2(), c0=poro, c1=perm, degree=0
    )
    #
    Phi = c[0]
    K = c[1]
    ...
    def boundary2(x):
        return (
            x[1] < DOLFIN_EPS or x[1] > DimY - DOLFIN_EPS
        )  # or (x[0] < DOLFIN_EPS and x[1] <= 0.48) or (x[0] < DOLFIN_EPS and x[1] >= 1-0.48)
    ...
    a = (
        mu / Phi * inner(grad(u), grad(tf1)) * dx
        + inner(grad(P), tf1) * dx
        + mu / K * inner(u, tf1) * dx
        + div(u) * tf2 * dx
    )
